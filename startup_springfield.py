...
def get_landcover(url):
    ...
    if landcover['name'] == '':
        ...
        runlog.p("20170213 Get landuse url = " % url)
...
def get_cachedprobmap(url):
    ...
    for probmap in cachedprobmaps:
        ...
        """
           Han - 20170220 - if [cannot find prob_res or prob_com] [open ONLY this cache become FAST! ]
           if comment out this 'return True', final value will use prob_maps, 
           otherwise they cannot use prob_maps because generated .bil files been delete 
        """
        #return True
...
def processDriverSets(driver):
    ...
    # jd - 2017-02-15 for download landuse map and rename to landuse.tif under folder ./Inputs/
    runlog.p("--20170215 - import road landuse map......")
    landuse_filename = site.saveFile(driver['landuse'], dir='./Inputs')
    print "hahahahah",landuse_filename
    os.rename('./Inputs/%s'%landuse_filename,'./Inputs/landuse.tif') 
...
def publishResults(title, site, resultsdir):
    ...
    # Add [nogrowth map] to the results layers on Google API (v2rast .zip)
    ...
    # 2017-02-15 - jd
    # Add [landuse map] to the results layers on Google API - jd - 20170213 (.tif and .map)
    # download landusemap.tif at processDriverSets() - jd - 20170213
    # publishSimMap("landuse", site, resultsdir+"/results", 'initial landuse map', nomin=True, flag=3)
    simmapTitleLU = "landuse"
    mapTitleLU = "landuse"
    mapbarTitleLU = "existing_landuse"
    simmapLU = "Inputs/%s.tif" % simmapTitleLU 
    mapfileLU = "Outputs/%s.map" % mapTitleLU
    url = resultsdir + "/results"    
    popattrurlLU = site.putSimMap("%s.tif" % simmapTitleLU, "%s.map" % mapTitleLU, url,
       	simmap_file = open(simmapLU, "rb"),
	mapfile_file = open(mapfileLU, "rb"))
    site.updateSimMap(popattrurlLU, title = mapbarTitleLU, description="landuse map")      
...
if __name__ == "__main__":
	...
	# os.system('rm ./Data/*.tif ./gluc/Data/*')
