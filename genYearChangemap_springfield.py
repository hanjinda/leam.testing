...
def genYearlyDemandGraph(demandstr, projid):
    print runlog.p(os.getcwd()+'/gluc/Data/projection.txt')
	
    pf = open(os.getcwd()+'/gluc/Data/projection.txt','w')
    #plines = pf.readlines()
    
    #for eachline in plines:
        #print eachline
    ...
    #pf.write(str(yr1)+","+str(yr2)+"\n")
    pf.write(str(pop1)+","+str(pop2)+","+str(emp1)+","+str(emp2)+"\n")
    #pf.write(str(emp1)+","+str(emp2)+"\n")

    # output increment demand per year relative to the start year.
    writeDemand(yearincrpoplist, yearincremplist, projid)
    runlog.h('20170408..............<br>'+str(yr1)+', '+str(pop1)+', '+str(pop2)+', <br>'+str(yr2)+', '+str(emp1)+', '+str(emp2)+', <br>'+str(yearincrpoplist)+', <br>'+str(yearincremplist))
    pf.close()
    return yr1, yr2
